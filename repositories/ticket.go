...
type TicketRepository interface {
	...
	SearchTicket(date string, startStationID, endStationID int) ([]models.Ticket, error)
...
func (r *repository) SearchTicket(date string, startStationsID, endStationID int) ([]models.Ticket, error) {
	var tickets []models.Ticket
	err := r.db.Where("start_date=? AND start_station_id=? AND destination_station_id=?", date, startStationsID, endStationID).Preload("StartStation").Preload("DestinationStation").Preload("User").Find(&tickets).Error
