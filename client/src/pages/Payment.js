                    ...
                    <Row>
                        <Col md={8}>
                            ...
                            <div className="border rounded">
                                <div className="px-3" style={{ width: 170, backgroundColor: '#ED7A9D', borderEndEndRadius: 50, borderTopLeftRadius: 3  }}><img className="me-2" src={LogoName}/><img src={Logo}/></div>
                                    ...
                                    <ul className="d-flex px-3 text-center" style={{ listStyle: "none", color: "grey" }}>
                                        ...
                                        <li className="px-4 ms-2" style={{ width: 180 }}>{mytransaction?.user?.name}</li>
                                        <li className="px-4 ms-4" style={{ width: 180 }}>{mytransaction?.user?.no_hp}</li>
                                        <li className="pe-4" style={{ width: 200 }}>{mytransaction?.user?.email}</li>
                                    </ul>
                                </div>
                            </div>
                            <div className="mt-4">
                                ...
                                <div className="border rounded mt-3" style={{ width: 500 }}>
                                    <ul className="d-flex p-4 justify-content-between" style={{ listStyle: "none" }}>
                                        <li className="me-5">{mytransaction?.ticket?.name_train} (Dewasa) x1</li>
                                    ...
                                    <ul className="d-flex px-4 py-2 my-0 rounded-bottom justify-content-between" style={{ listStyle: "none", backgroundColor: '#E6E7E7' }}>
                                        ...
                                        <li className="me-3 fw-bold" style={{ fontSize: 20 }}>Rp. {mytransaction?.ticket?.price}</li>
                                    </ul>
                                </div>
                            </div>
                            <button className="border-0 text-white py-2 rounded my-3 fw-bold" onClick={() => handleBuy.mutate(id)} style={{ width: 500, backgroundColor: '#ED7A9D' }}>Bayar Sekarang</button>
                        </Col>
                        <Col md={4}>
                            <div className="d-flex justify-content-between px-3 pt-3" style={{ backgroundColor: "#D1D1D0" }}>
                                <div>
                                    <h2 className="fw-bold">Kereta Api</h2>
                                    <p style={{ fontSize: 18 }}>Senin, <span className="text-secondary opacity-75">{mytransaction?.ticket?.start_date}</span></p>
                                </div>
                                <div>
                                    <img src={QrLogo}/>
                                    <p>INV0101</p>
                            ...
                            <div className="p-3" style={{ backgroundColor: '#F4F5F4', height: 300 }}>
                                <div>
                                    <h4 className="fw-bold">{mytransaction?.ticket?.name_train}</h4>
                                ...
                                <div className="d-flex">
                                    ...
                                    <ul style={{ listStyle: "none" }}>
                                        <li className="mt-3 mb-5">
                                            <h5 style={{ lineHeight: 0.001 }}>{mytransaction?.ticket?.start_time}</h5>
                                            <span className="text-secondary opacity-75">{mytransaction?.ticket?.start_date}</span>
                                        </li>
                                        <li className="mt-5 pt-4">
                                            <h5 style={{ lineHeight: 0.001 }}>{mytransaction?.ticket?.arrival_time}</h5>
                                            <span className="text-secondary opacity-75">{mytransaction?.ticket?.start_date}</span>
                                        </li>
                                    </ul>
                                    <ul className="" style={{ listStyle: "none" }}>
                                        <li className="mt-3 mb-5">
                                            <h5 style={{ lineHeight: 0.001 }}>{mytransaction?.ticket?.start_station.name}</h5>
                                            <span className="text-secondary opacity-75">Stasiun {mytransaction?.ticket?.start_station.name}</span>
                                        </li>
                                        <li className="mt-5 pt-4">
                                            <h5 style={{ lineHeight: 0.001 }}>{mytransaction?.ticket?.destination_station.name}</h5>
                                            <span className="text-secondary opacity-75">Stasiun {mytransaction?.ticket?.start_station.name}</span>
                                        </li>
                                    </ul>
                                </div>
