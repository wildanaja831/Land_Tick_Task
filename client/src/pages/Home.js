...
function Home() {
  ...
  let {data: tickets, refetch} = useQuery("ticketsCache", async () => {
    const response = isSearch ? (await API.get(`/ticket?date=${form.date}&startStation=${form.start_station}&destinationStation=${form.destination_station}&qty=${parseInt(form.adult) + parseInt(form.child)}`))
  ...
  return (
      <div className="font-all" style={{ minHeight: '100vh', display: "flex", flexDirection: "column" }}>
        ...
        <div className='bg-light'>
          <MainContents submitData={handleSubmitSearchTicket} emptyData={handleSubmitEmpty} />
        ...
        <footer style={{ height: 50, backgroundColor: '#ED7A9D', marginTop: 'auto' }}></footer>
      </div>
  );
}
